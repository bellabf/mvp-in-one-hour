...
class Recipe(BaseModel):
    ...
    @classmethod
    def validate_ingredient_count(cls, v: List[Ingredient]):
        if len(v) != SETTINGS.ingredients_per_recipe:
            raise ValueError(
                "Recipe must have exactly "
                f"{SETTINGS.ingredients_per_recipe} ingredients, got {len(v)}"
            )
        return v
...
class RecipesOnly(BaseModel):
    """Weekly meal plan recipes without shopping list."""
    ...
    @classmethod
    def validate_recipe_count(cls, v: List[Recipe]):
        if len(v) != 7:
    ...
    @field_validator("recipes")
    @classmethod
    def validate_unique_themes(cls, v: List[Recipe]):
